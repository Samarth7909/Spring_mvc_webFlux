 ...
 * on 15/08/2025
 * 
 * @description: This is yet another bad WebSocket handler.It is supposed to
 *               store references to each stream that sends
 *               messages to the client, but by providing the stream ourselves,
 *               Spring is not involved anymore, so no subscriber is
...
public class WorseTechNewsHandler implements WebSocketHandler {
	...
	private Flux<WebSocketMessage> getRandomNews(final String message, final WebSocketSession session) {
		...
		return Flux
				.create(fluxSink -> {
					...
					var data = Flux
							.generate((SynchronousSink<String> synchronousSink) -> synchronousSink
									.next(BookNewReleasesUtil.randomNews()))
							.map(session::textMessage).delayElements(Duration.ofSeconds(rate));
	...
	public Mono<Void> handle(WebSocketSession session) {
		return session.send(session.receive()
				...
				.flatMap(message -> this.getRandomNews(message, session))).then();
	}
}
